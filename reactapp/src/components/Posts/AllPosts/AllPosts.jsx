...
const AllPosts = () => {
    ...
    useEffect(() => {
        const check = async () => {
            try {
                ...
                const response = await axios.get(`/post/GetAllPosts?PageNumber=1&PageSize=${pageSize}`);
    ...
    const getCountOfPosts = async () => {
        try {
            const response = await axios.get(`/post/GetCountPosts`);
    ...
    const handlePageChange = async (selectedPage) => {
        ...
        axios.defaults.headers.common['xAuthAccessToken'] = localStorage.getItem('accessToken');
        const response = await axios.get(`/post/GetAllPosts?PageNumber=${selectedPage.selected + 1}&PageSize=5`);
        delete axios.defaults.headers.common['xAuthAccessToken'];
        setPosts(response.data);
